...
int main(int argc, char **argv)
{
  ...
  else
  {
   ...
   int state  = 0;
    bc.moveRobot(1000.0, 45.0);
   FILE *p1;
   FILE *p2;
   FILE *p3;
   p1 = fopen("/home/bhargav/Mbr/xy.txt", "w");
  for(int j=1;j<=4;j++)
  {
   p1 = fopen("/home/bhargav/Mbr/xy.txt", "w");
   p2 = fopen("/home/bhargav/Mbr/dist.txt", "w");
   p3 = fopen("/home/bhargav/Mbr/angle.txt", "w");
  vector<int> readings = bc.getReadings();
  vector<int> angles = bc.getAngles();
 // state = 1;
  fprintf(p1,"%d\t",j);
  fprintf(p1,"%d\t",xc);
  fprintf(p1,"%d\t",yc);
  ...
  for( int i=0; i<angles.size(); i++ )
  {
    ...
    fprintf(p3,"%d\t", angles[i]);
    printf("%d\t",angles[i]);
  }
    fclose(p3);
   bc.moveRobot(1000.0, 0.0);
  ArUtil::sleep(5000);
  }
 // fclose(p1);
